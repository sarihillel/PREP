...
var checklistRecords = function (options) {
    ...
    this.functions.clearValidQuestionsFromQuestionsContainer = function () {
        $.each(self.records, function (index, record) {
            if (record.IsValidate || !record.IsChangedRecord) {
                if (record.IsView) {
                    $(".questions-container").find("[data-record-key=" + record[self.record.key] + "]").hide();
                }
                record.IsFilter = false;
                record.IsView = false;
                self.ArrayTotalByriskLevel[record[self.record.RiskLevelID]]--;
                self.TotalRecords--;
            }
        });
    }
    ...
    this.functions.SaveRecords = function (callbackdone) {
        ...
        $.each(self.records, function (index, element) {
            if (element.IsView && element.IsChangedRecord) {
                ...
                if (element.IsChangedRecord && element.IsValidate) {
        ...
        if (callbackdone)
            ...
            if (CountValidate == 0) {
                ...
                confirm("Only part of your answers were saved - please review invalid answers, verify that all mandatory fields are filled and that there is no mismatch between Actual Completion & Risk Level",
                    warning, _SaveRecords, [allrecords, null, CountNotValidate]);
            }
            else {
...
var ShowHideCheckListdetails = function () {
    ...
    $(this).closest("div._border_bottom").find(".hide-row").slideToggle('slow');
    //renderSubArea($(this));
}
